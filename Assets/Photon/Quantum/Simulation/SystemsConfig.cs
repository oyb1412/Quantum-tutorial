namespace Quantum {
  ...
  public partial class SystemsConfig : AssetObject {
    ...
    private static SystemBase Create<ChildrenType>(Type type, string name, ChildrenType[] children) where ChildrenType : SystemBase {
      // Conventions and priority are: (name, children), (name), ()
      if (type.GetConstructor(BindingFlags.Instance | BindingFlags.Public, null, new Type[] { typeof(string), typeof(ChildrenType[]) }, null) != null) {
        return Activator.CreateInstance(type, name, children) as SystemBase;
      } else if (type.GetConstructor(BindingFlags.Instance | BindingFlags.Public, null, new Type[] { typeof(string) }, null) != null) {
